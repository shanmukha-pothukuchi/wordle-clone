...
const handleKeyClick = (e) => {
  const event = new CustomEvent("virtualKeyDown", {
    detail: {
      key:
        e.target.getAttribute("data-key").length === 1
          ? e.target.getAttribute("data-key")
          : capitalize(e.target.getAttribute("data-key")),
    },
  });
...
let renderRow = async (rowIndex, cells, corectAnswer = false, word = null) => {
  ...
  for (let i = 0; i < grid[rowIndex].length; i++) {
    ...
    if (cells[i] === "correct_place_correct_letter") {
      ...
      await sleep(100);
      continue;
    } else if (cells[i] === "wrong_place_correct_letter") {
      cell.classList.add("wrong_place_correct_letter");
      await sleep(100);
      continue;
    } else if (cells[i] === "wrong_place_wrong_letter") {
      cell.classList.add("wrong_place_wrong_letter");
      await sleep(100);
      continue;
    }
  }
  ...
  if (corectAnswer) {
    for (let i = 0; i < grid[rowIndex].length; i++) {
      const cell = grid[rowIndex][i];
      cell.classList.add("finished_word");
      await sleep(75);
    }
  }
};

let renderKeyboard = (alphabetmapping) => {
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    const letter = key.getAttribute("data-key");

    switch (alphabetmapping[letter]) {
      case "true":
...
let handleKeydown = (e) => {
  ...
  if (key === "Enter") {
    ...
    // "true" -> green // correct place, correct letter
    // "false" -> yellow // wrong place, correct letter
    // "undefined" -> grey // wrong place, wrong leter
    // null -> default // not sure yet
    let alphabetmapping = {
      a: null,
      b: null,
      c: null,
    ...
    for (let i = 0; i < guess.length; i++) {
      ...
      if (solution.includes(letter)) {
        if (alphabetcount[letter] > 0) {
          if (eval[i] === undefined) {
            eval[i] = "wrong_place_correct_letter";
            alphabetcount[letter]--;
            alphabetmapping[letter] = "false";
          }
        }
      }
    }
    ...
    for (let i = 0; i < guess.length; i++) {
      ...
      if (
        ...
        if (eval[i] === undefined) {
          eval[i] = "wrong_place_wrong_letter";
          alphabetcount[letter]--;
          alphabetmapping[letter] = "undefined";
        }
      }
    }
  ...
  if (key === "Backspace") {
    for (let i = grid[row].length - 1; i >= 0; i--) {
      ...
      if (cell.textContent.length > 0) {
        cell.textContent = "";
        cell.classList.remove("type-animation");
        break;
      }
    }
  }
  ...
  for (cell of grid[row]) {
    if (cell.textContent === "") {
      cell.textContent = key;
      cell.classList.add("type-animation");
      break;
    }
  }
};
...
document.addEventListener("virtualKeyDown", handleKeydown);
